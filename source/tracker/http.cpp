...
http::http(const url_t& url):socket(url.host,url.port),url(url),n_args(0){
    if(url.protocol!=url_t::HTTP){
        throw runtime_error("url is not http");
    }
    this->url=url;
...
string http::urlencode(const buffer &b) {
    ...
    for(int i=0;i<b.size();i++){
        char c=b[i];
        if(isalnum(c) || c=='-' || c=='_' || c=='.' || c=='~') ans.push_back(c);
        else{
            ans.push_back('%');
            ans.push_back(lookUp[(c&0xF0)>>4]);
            ans.push_back(lookUp[(c&0xF0)]);
        }
    }
    return ans;
}
